...
class RobotTest < Minitest::Test
  ...
  def test_reset_multiple_times
    ...
    assert_equal names.uniq.size, 5
  end
  ...
  def test_generate_all_robots
    ...
    assert_equal all_names_count, robots.size,
                 'Timed out trying to generate all possible robots'
    assert seen_names.values.all? { |count| count == 1 },
           'Some names used more than once'
    assert seen_names.keys.all? { |name| name.match(NAME_REGEXP) },
           "Not all names match #{NAME_REGEXP}"
  end
end
